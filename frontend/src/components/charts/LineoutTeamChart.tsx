...
const LineoutTeamChart = ({ events, onChartClick }) => {
  ...
  const won = teamEvents.filter(event => {
  ...
  const lost = teamEvents.filter(event => {
    const result = getResult(event);
    return result && (
...
export default LineoutTeamChart;
