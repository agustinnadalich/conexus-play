...
const TurnoversRecoversBarChart = ({ events, onChartClick }: any) => {
  ...
  useEffect(() => {
    ...
    const recoverEvents = events.filter(event => event.CATEGORY === 'TURNOVER+' || event.event_type === 'TURNOVER+');
    const playerLabels = [...new Set(recoverEvents.map(getPlayerName).filter(p => p !== null))].sort();
      ...
      datasets: [
        {
          ...
          data: playerLabels.map(player => 
            recoverEvents.filter(event => getPlayerName(event) === player).length
          ),
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Turnovers Recuperados por Jugador',
      },
    },
    scales: {
...
export default TurnoversRecoversBarChart;
